...
3. **历史分层策略（核心逻辑）**：
    - [最近经历]只保留最近3次互动的细节。 
    - [重要里程碑]起归档作用，保留关键历史。该板块不要记录日常琐事。对于已记录的里程碑历史除非认为其过于流水账，否则非必要不删除。
4. **遗忘原则**：
...
7. **禁止**使用如“新增”、“状态更新”等前缀，不要保留重复条目。

### 目标输出结构（严格遵守格式）：

[最近经历](顺序排列，仅保留2条【近期】条目，1条【最新】条目)
- 【近期】... (一句话概括)
- 【近期】... (一句话概括)
- 【最新】... (保留关键细节，限100字)
...
**系统指令：执行增量更新与无损压缩。**
1. 将所有描述转化为关键词标签，避免完整句子。
2. 若[当前场景]/[当前人物状态]无变化，请尽量**保持原文不动**（delta only），不要每次都润色改写。
3. 移除[待办]中的已完成事项。
