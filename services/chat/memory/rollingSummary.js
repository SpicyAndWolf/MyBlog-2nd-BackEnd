...
function buildRollingSummaryPrompt({ previousSummary, newMessages, maxChars }) {
  ...
  const system = `
你是一个**高密度信息压缩引擎**。你的任务是将“剧情文本”转化为**极其紧凑的数据库条目**。
...
3.  **去重与合并**：
    - 对于[关键共同经历]，**早期事件**必须压缩为一句话（如："[早期] 确认关系 -> 首次约会（xx地点） -> xxx）。
    - 只有**最近3次**交互可以保留细节描写。
...
- 【最新】... (可保留细节)
- 【近期】...
- 【历史档案】(此处将旧经历高度浓缩为事件链，如：事件A -> 事件B -> 事件C->...，此部分按时间轴顺序概括)
[待办](仅记录未完成事项)
- ...
[当前场景](环境/感官/位置 - 必须保留)
- ...
[当前人物状态](动作/情绪/衣着 - 必须保留)
- ...
`.trim();
...
**系统指令：执行增量更新与无损压缩。**
1. 将所有描述性文字转化为**关键词标签**。
2. 将早于“最近3次”的互动折叠入【历史档案】。
3. 严格控制总字数在 **${charBudget}** 字符以内。
`.trim();
